...
public class Deck {
    ...
    private void createShuffledDeck( ) {
        ...
        for (int i = 0; i < mOrderedDeck.length; i++) {
            ...
            boolean newIndex;
            do {
                randomIndex = (int) (mOrderedDeck.length * Math.random());
                newIndex = true;
                for (int j = 0; j < mOrderedDeck.length; j++) {
                    if (indices[j] == randomIndex) newIndex = false;
                }
            } while (!newIndex);
            indices[indicesIndex] = randomIndex;
            indicesIndex++;
        }
        for (int i = 0; i < indices.length; i++) {
            if (indices[i] == 0) continue;
            mShuffledDeck.add(mOrderedDeck[indices[i]]);
        }
    }
    ...
    protected Card getTopCard( ) {
        if (mShuffledDeck.size() > 0) return mShuffledDeck.remove(0);
